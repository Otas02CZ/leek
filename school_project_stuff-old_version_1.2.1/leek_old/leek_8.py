...
def create_viewable():
    ...
    if location=="root":
        ...
        for i in range(len(search_result)):
            viewable[i] = get_file_record_as_dictionary(search_result, i)
    else:
...
def do_sort():
    ...
    match sort_key:
        case "none":
            ...
            return
        case "name":
            viewable_backup = sorted(viewable.items(), key=lambda kv: kv[4])
            print(viewable)
            input()
        case "created":
            viewable_backup = sorted(viewable.items(), key=lambda kv: (viewable[kv]["created"]))
            viewable = viewable_backup.copy()
            print(viewable)
            input()
